...
exports.renderFile = function(path, options, source){
  ...
  if(typeof options.render === 'undefined'){
    options.render = function (subpath, options) {
      return exports.renderFile(subpath, options, path);
    };
  }
  ...
  return Q.resolve().then(function(){
    return resolverRead(options.cache, path, source);
      ...
      var inner = exports.render(str, options);
      if (inherits) {
        var parentoptions = Object.create(options);
        parentoptions.contents = inner;
        return exports.renderFile(inherits, parentoptions, path);
      } else {
        return inner;
      }
  });
...
function resolverRead(cacheEnabled, to, from){
  ...
  function next(){
    ...
    if(path){
      paths.push('      - '+path);
      return readFile(path).fail(function(){
        return next();
      });
    } else {
