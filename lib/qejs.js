 ...
 * @api public
 */
exports.renderFile = function(name, options){
  Q.resolve().then(function(){
    var fs;
    try{
      fs = require('fs');
    }catch(ex){
      throw new Error('Render File only works in node.js');
    }
    var fileText = Q.defer();
    fs.readFileSync(name, 'utf-8',function(text){
      fileText.resolve(text);
    });
...
exports.renderFile = function(path, options){
  ...
  return Q.resolve().then(function(){
    ...
    if(options.cache){
      return cache[key] || read(path, 'utf8').then(function(text){return (cache[key] = text);});
    } else {
